class DartboardRenderer {
    ...
    drawSector(i, center, radius) {
        ...
        this.drawRing(center, radius, startAngle, endAngle, 
                     this.ringRadii.outerBull, this.ringRadii.innerSingle, 
                     color, i, 'innerSingle');

        // Draw outer single area
        this.drawRing(center, radius, startAngle, endAngle, 
                     this.ringRadii.tripleOuter, this.ringRadii.doubleInner, 
                     color, i, 'outerSingle');

        // Draw triple ring
        this.drawRing(center, radius, startAngle, endAngle, 
                     this.ringRadii.tripleInner, this.ringRadii.tripleOuter, 
                     i % 2 === 0 ? this.COLORS.RED : this.COLORS.GREEN, i, 'triple');

        // Draw double ring
        this.drawRing(center, radius, startAngle, endAngle, 
                     this.ringRadii.doubleInner, this.ringRadii.doubleOuter, 
                     i % 2 === 0 ? this.COLORS.RED : this.COLORS.GREEN, i, 'double');
    ...
    drawRing(center, radius, startAngle, endAngle, inner, outer, color, index, ringType) {
        ...
        if (this.hoverInfo && this.hoverInfo.index === index && this.hoverInfo.ring === ringType) {
            this.ctx.shadowBlur = ringType === 'bull' ? 30 : 
                                 ringType === 'outerBull' ? 25 : 
                                 ringType === 'triple' || ringType === 'double' ? 20 : 15;
            this.ctx.shadowColor = 'rgba(255,255,0,0.7)';
            this.ctx.fillStyle = 'rgba(255,255,0,0.6)';
        } else {
            this.ctx.shadowBlur = ringType === 'bull' ? 15 : 
                                ringType === 'outerBull' ? 10 : 
                                ringType === 'triple' || ringType === 'double' ? 5 : 0;
            this.ctx.shadowColor = color === this.COLORS.RED ? 'rgba(255,0,0,0.5)' : 
                                color === this.COLORS.GREEN ? 'rgba(0,255,0,0.5)' : 'transparent';
    ...
    getThrowFromClick(x, y) {
        ...
        const color = info.ring === 'bull' ? '#ff0000' :
                     info.ring === 'outerBull' ? '#00ff00' :
                     info.ring === 'triple' || info.ring === 'double' ? '#ffff00' : '#ffffff';
        this.createParticles(x - rect.left, y - rect.top, color);

        return new Throw(clickX, clickY, info.score === 0 ? 0 : this.numberOrder[info.index], 
                        info.score === 50 ? 2 : 
                        info.score === 25 ? 1 : 
                        info.ring === 'triple' ? 3 : 
                        info.ring === 'double' ? 2 : 1, 
                        info.score);
    }
